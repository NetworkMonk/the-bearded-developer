...
export default function Toggle({ disabled, onChange, checked }) {
  return (
    <Switch
      ...
      checked={checked}
      onChange={onChange}
      className={`group relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent bg-gray-200 transition-colors duration-200 ease-in-out focus:outline-hidden data-[checked]:bg-brand-blue ${
        disabled ? "opacity-50" : ""
      }`}
    >
